...
class SpeedometerMonitor:
    ...
    def create_widgets(self):
        ...
        # CPU Speedometer with 20 LEDs
        cpu_container = tk.Frame(top_frame, bg=self.colors['bg'])
        cpu_container.pack(side=tk.LEFT, padx=20)
        
        # CPU LED Frame (Left side of speedometer)
        self.cpu_led_frame = tk.Frame(cpu_container, bg=self.colors['bg'], width=40, height=300)
        self.cpu_led_frame.pack(side=tk.LEFT, fill=tk.Y)
        self.cpu_led_frame.pack_propagate(False)
        
        # CPU Canvas
        cpu_canvas_frame = tk.Frame(cpu_container, bg=self.colors['bg'])
        cpu_canvas_frame.pack(side=tk.LEFT)
        self.cpu_canvas = tk.Canvas(cpu_canvas_frame, width=300, height=300, 
                                   bg=self.colors['bg'], highlightthickness=0)
        self.cpu_canvas.pack()
        tk.Label(cpu_canvas_frame, text="CPU USAGE", font=("Arial", 12, "bold"), 
                fg=self.colors['text'], bg=self.colors['bg']).pack(pady=5)
        
        # Memory Speedometer with 20 LEDs
        memory_container = tk.Frame(top_frame, bg=self.colors['bg'])
        memory_container.pack(side=tk.RIGHT, padx=20)
        
        # Memory Canvas
        memory_canvas_frame = tk.Frame(memory_container, bg=self.colors['bg'])
        memory_canvas_frame.pack(side=tk.LEFT)
        self.memory_canvas = tk.Canvas(memory_canvas_frame, width=300, height=300, 
                                      bg=self.colors['bg'], highlightthickness=0)
        self.memory_canvas.pack()
        tk.Label(memory_canvas_frame, text="MEMORY USAGE", font=("Arial", 12, "bold"), 
                fg=self.colors['text'], bg=self.colors['bg']).pack(pady=5)
        
        # Memory LED Frame (Right side of speedometer)
        self.memory_led_frame = tk.Frame(memory_container, bg=self.colors['bg'], width=40, height=300)
        self.memory_led_frame.pack(side=tk.LEFT, fill=tk.Y)
        self.memory_led_frame.pack_propagate(False)
        
        # Bottom row - Disk and Network
        bottom_frame = tk.Frame(dashboard_frame, bg=self.colors['bg'])
        bottom_frame.pack(fill=tk.X, pady=10)
        
        # Disk Speedometer with 20 LEDs
        disk_container = tk.Frame(bottom_frame, bg=self.colors['bg'])
        disk_container.pack(side=tk.LEFT, padx=20)
        
        # Disk LED Frame (Left side of speedometer)
        self.disk_led_frame = tk.Frame(disk_container, bg=self.colors['bg'], width=40, height=300)
        self.disk_led_frame.pack(side=tk.LEFT, fill=tk.Y)
        self.disk_led_frame.pack_propagate(False)
        
        # Disk Canvas
        disk_canvas_frame = tk.Frame(disk_container, bg=self.colors['bg'])
        disk_canvas_frame.pack(side=tk.LEFT)
        self.disk_canvas = tk.Canvas(disk_canvas_frame, width=300, height=300, 
                                    bg=self.colors['bg'], highlightthickness=0)
        self.disk_canvas.pack()
        tk.Label(disk_canvas_frame, text="DISK USAGE", font=("Arial", 12, "bold"), 
                fg=self.colors['text'], bg=self.colors['bg']).pack(pady=5)
        
        # Network Speedometer with 20 LEDs
        network_container = tk.Frame(bottom_frame, bg=self.colors['bg'])
        network_container.pack(side=tk.RIGHT, padx=20)
        
        # Network Canvas
        network_canvas_frame = tk.Frame(network_container, bg=self.colors['bg'])
        network_canvas_frame.pack(side=tk.LEFT)
        self.network_canvas = tk.Canvas(network_canvas_frame, width=300, height=300, 
                                       bg=self.colors['bg'], highlightthickness=0)
        self.network_canvas.pack()
        tk.Label(network_canvas_frame, text="NETWORK UPLOAD", font=("Arial", 12, "bold"), 
                fg=self.colors['text'], bg=self.colors['bg']).pack(pady=5)
        
        # Network LED Frame (Right side of speedometer)
        self.network_led_frame = tk.Frame(network_container, bg=self.colors['bg'], width=40, height=300)
        self.network_led_frame.pack(side=tk.LEFT, fill=tk.Y)
        self.network_led_frame.pack_propagate(False)
        ...
        self.led_status_label = tk.Label(status_frame, text="●", font=("Arial", 12),
                                        fg=self.colors['led_green'], bg=self.colors['dial_bg'])
        self.led_status_label.pack(side=tk.RIGHT, padx=10)
        
        # Draw initial speedometers and LEDs
    ...
    def draw_external_leds(self, led_frame, value, label):
        """Draw 20 external LED indicators on the side frames"""
        # Clear the frame
        ...
        total_height = (led_height * num_leds) + (led_spacing * (num_leds - 1))
        frame_height = 300
        ...
        for i in range(num_leds):
            # Determine LED color based on position (0-19)
            ...
            y_pos = start_y + (i * (led_height + led_spacing))
            
            # Create LED
            led = tk.Frame(led_frame, bg=led_color, width=led_width, height=led_height,
                          relief='raised', bd=1)
            led.place(x=5, y=y_pos, width=led_width, height=led_height)
    ...
    def start_led_blink(self):
        ...
        def blink():
            while self.monitoring:
                ...
                # Update all external LEDs
                self.root.after(0, self.update_external_leds)
    ...
    def update_external_leds(self):
        """Update all external LED indicators"""
        self.draw_external_leds(self.cpu_led_frame, self.cpu_usage, "CPU")
        self.draw_external_leds(self.memory_led_frame, self.memory_usage, "MEM")
        self.draw_external_leds(self.disk_led_frame, self.disk_usage, "DISK")
        self.draw_external_leds(self.network_led_frame, self.network_upload, "NET")
    ...
    def update_display(self, cpu, memory, disk, network, memory_obj, disk_obj, download_speed):
        ...
        if max_usage > 90:
            status = "CRITICAL"
            color = "#ff0000"
            led_color = self.colors['led_red']
        elif max_usage > 70:
            status = "HIGH LOAD"
            color = "#ffaa00"
            led_color = self.colors['led_yellow']
        elif max_usage > 50:
            status = "MODERATE"
            color = "#ffff00"
            led_color = self.colors['led_yellow']
            ...
            status = "NORMAL"
            color = "#00ff00"
            led_color = self.colors['led_green']
        
        self.status_label.config(text=f"SYSTEM STATUS: {status}", fg=color)
        self.led_status_label.config(fg=led_color)
